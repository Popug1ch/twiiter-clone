...
class TestTweetsAPI:
    def test_create_tweet(self, client, auth_headers, sample_tweet_data):
        """Создание твита."""
        with patch("app.routers.tweets.tweet_crud.create_tweet", new=AsyncMock(return_value=123)):
            resp = client.post("/api/tweets", headers=auth_headers, json=sample_tweet_data)
    ...
    def test_delete_tweet(self, client, auth_headers):
        ...
        with patch("app.routers.tweets.tweet_crud.delete_tweet", new=AsyncMock(return_value=True)):
    ...
    def test_delete_not_found(self, client, auth_headers):
        ...
        with patch("app.routers.tweets.tweet_crud.delete_tweet", new=AsyncMock(return_value=False)):
    ...
    def test_like_unlike(self, client, auth_headers):
        """Лайк / снятие лайка."""
        with patch("app.routers.tweets.tweet_crud.like_tweet", new=AsyncMock(return_value=None)):
            resp1 = client.post("/api/tweets/123/likes", headers=auth_headers)
        assert resp1.status_code == status.HTTP_200_OK
        assert resp1.json()["result"] is True

        with patch("app.routers.tweets.tweet_crud.unlike_tweet", new=AsyncMock(return_value=None)):
    ...
    def test_feed(self, client, auth_headers, sample_feed_data):
        """Лента твитов."""
        with patch("app.routers.tweets.tweet_crud.get_feed_for_user", new=AsyncMock(return_value=sample_feed_data)):
